...
class ProfileAppBar extends StatefulWidget {
  ...
  const ProfileAppBar({Key? key, this.nameColor = lightyellow})
      : super(key: key);

  @override
  State<ProfileAppBar> createState() => _ProfileAppBartState();
}

class _ProfileAppBartState extends State<ProfileAppBar> {
  @override
          ...
          child: Row(
            children: [
              Padding(
                ...
                child: Text(snapshot.data!["nome"].toString().split(" ").first,
                    style: GoogleFonts.barlowCondensed(
                        color: widget.nameColor, fontSize: 22)),
              ),
              Container(
                  width: 40,
                  height: 40,
                  ...
                  child: (snapshot.data!["imagem"] != null)
                      ...
                      : ClipRRect(
                          borderRadius:
                              const BorderRadius.all(Radius.circular(70)),
                          child: (snapshot.data!["categoria"] == "Supervisor")
                              ? Image.asset(
                                  "assets/users/supervisor.png",
                                  fit: BoxFit.cover,
                                )
                              : (snapshot.data!["categoria"] == "Estoque")
                                  ? Image.asset(
